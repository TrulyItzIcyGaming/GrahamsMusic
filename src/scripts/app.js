...
function displayAlbums(albums, musicGallery) {
    ...
    albums.forEach(album => {
        ...
        for (const format of imageFormats) {
            const imagePath = `music/${album.album}/cover.${format}`;
        ...
        albumImage.src = albumImageSrc || 'default-cover.jpg'; // Fallback to a default image if no cover is found
...
function displayAlbum(album, albums, musicGallery) {
    ...
    album.files.forEach(file => {
        ...
        audio.controls = true;
        audio.src = `music/${album.album}/${file}`;
